...
function(a, b) {
	...
	var c = Math.round,
		...
		w = {},
		x = function(b, c) {
			var d = a.slice.call(b.touches || b),
				e = b.type,
				f = [],
				g = [];
			if (e !== a.EVENT_START && e !== a.EVENT_MOVE || 1 !== d.length) {
				var h = 0,
					f = [],
					g = [],
					i = a.slice.call(b.changedTouches || b);
				c.target = b.target;
...
function(a) {
	...
	a.openWindow = function(b, c, f) {
		if (window.plus) {
			...
			if (f.createNew !== !0 && (g = plus.webview.getWebviewById(c))) return h = a.showOptions(f.show), g.show(h.aniShow, h.duration, function() {
				d(g), e(g, "pagebeforeshow", !1)
			}), g;
		...
		n = a.Slider = a.Scroll.extend({
			...
			_handleSlide: function(b) {
				...
				if (b.target === c.wrapper) {
					...
					var e = c.scroller.querySelectorAll(k),
						f = d.slideNumber;
					...
					if (l) {
						...
						if (m.length > 0)
							...
							if (n) n.innerText = d.slideNumber + 1;
								...
								for (var o = c.wrapper.querySelectorAll(".mui-control-item"), g = 0, h = o.length; h > g; g++) o[g].classList[g === d.slideNumber ? "add" : "remove"](j)
						}
					}
					b.stopPropagation()
				}
				...
				switch (b.type) {
					...
					case "dragend":
						if (this.isDragging) {
							...
							if (this.slideIn) {
								if (e = f >= 0 ? this.offCanvasRightWidth && f / this.offCanvasRightWidth || 0 : this.offCanvasLeftWidth && f / this.offCanvasLeftWidth || 0, e >= .5 && "left" === d ? this.openPercentage(0) : e > 0 && .5 >= e && "left" === d ? this.openPercentage(-100) : e >= .5 && "right" === d ? this.openPercentage(0) : e >= -.5 && 0 > e && "left" === d ? this.openPercentage(100) : e > 0 && .5 >= e && "right" === d ? this.openPercentage(-100) : -.5 >= e && "right" === d ? this.openPercentage(0) : e >= -.5 && "right" === d ? this.openPercentage(100) : -.5 >= e && "left" === d ? this.openPercentage(0) : e >= -.5 && "left" === d ? this.openPercentage(-100) : this.openPercentage(0), 1 === e || -1 === e || 0 === e) return void this._dispatchEvent()
							} else {
...
function(a) {
	var b = "ontouchstart" in document,
		...
		d = "change",
		e = "mui-numbox",
		f = "mui-numbox-btn-plus",
		g = "mui-numbox-btn-minus",
		h = "mui-numbox-input",
